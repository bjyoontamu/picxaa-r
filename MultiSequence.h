...
public:
	...
	~MultiSequence() {
		...
		if (sequences) {
			...
			// free all sequences
			for (SafeVector<Sequence *>::iterator iter = sequences->begin(); iter
					!= sequences->end(); ++iter) {
				assert (*iter);
				delete *iter;
				*iter = NULL;
	...
	void ParseMSF(FileBuffer &infile, string header, bool stripGaps = false) {
		...
		while (!infile.eof() && header.find("..", 0) == string::npos) {
			if (header.find("CLUSTAL", 0) == 0 || header.find("PROBCONS", 0)
					== 0) {
				clustalW = true;
				break;
			}
		...
		while (valid) {
			...
			if (in >> word) {
				...
				if (clustalW) {
					if (!isspace(header[0]) && find(seqNames.begin(),
							seqNames.end(), word) == seqNames.end()) {
		...
		for (int i = 0; i < (int) seqNames.size(); i++) {
			...
			Sequence *seq = new Sequence(seqData[i], seqNames[i],
					seqLengths[i], i, i);
	...
	void WriteMFA(ostream &outfile, string *ssCons = NULL, int numColumns = 60,
	...
	void WriteMFA2(ostream &outfile, string *ssCons = NULL, int numColumns = 60,
			bool useIndices = false) {
		if (!sequences)
			return;
	...
	void WriteALN(ostream &outfile, int numColumns = 60) {
		...
		outfile << "PicXAA-R (1.0) multiple RNA sequence alignment" << endl;
		outfile << "//" << endl;
		...
		while (!allDone) {
			...
			for (int i = 0; i < GetNumSequences(); i++) {
				...
				if (writtenChars < lengths[i]) {
					...
					for (int j = 0; j < longestComment
							- (int) GetSequence(i)->GetName().length(); j++)
						outfile << ' ';
	...
	// number of columns for the output.
	///////////////////////////////////////////////////////////////
	void WriteWEB(ostream &outfile, string *ssCons = NULL, int numColumns = 60,
			bool useIndices = false) {
		if (!sequences)
			return;
		...
		for (int i = 1; i < length; i++) {
			...
			if (ssCons->at(i) == '(') {
				...
				outfile << "<segment position=\"" << segmentPos++
						<< "\" starts=\"" << i << "\"" << " ends=\"" << j
						<< "\"/>" << endl;
	...
	// number of columns for the output.
	///////////////////////////////////////////////////////////////
	void WriteSTOCKHOLM(ostream &outfile, string *ssCons = NULL,
			int numColumns = 60) {
		if (!sequences)
			return;
			...
			if (ssCons != NULL) {
				...
				for (int j = 0; j < numColumns; j++) {
					if (ssCons->at(writtenChars + j + 1) == '(')
						...
						outfile << ".";
					if ((unsigned int) writtenChars + j + 1 >= ssCons->length()
							- 1)
						break;
				}
				outfile << endl;
	...
	// number of columns for the output.
	///////////////////////////////////////////////////////////////
	void WriteWebSTOCKHOLM(ostream &outfile, string *ssCons = NULL,
			int numColumns = 60) {
		if (!sequences)
			return;
	...
	// number of columns for the output.
	///////////////////////////////////////////////////////////////
	void WriteMXSCARNA(ostream &outfile, string *ssCons = NULL, int numColumns =
			60) {
		if (!sequences)
			return;

		outfile << "PicXAA-R version 1.0 " <<
	...
	MultiSequence *Project(const set<int> &indices) {
		...
		// grab old data
		int i = 0;
		for (set<int>::const_iterator iter = indices.begin(); iter
				!= indices.end(); ++iter) {
		...
		for (set<int>::const_iterator iter = indices.begin(); iter
				!= indices.end(); ++iter) {
			ret->AddSequence(new Sequence(newPtrs[i++],
					GetSequence(*iter)->GetHeader(), newLength, GetSequence(
							*iter)->GetSortLabel(),
